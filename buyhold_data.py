...
def buyhold_benchmark(df: pd.DataFrame,starting_equity: float,instrument_type : str,risk_free_investment: float = 0.04):
    ...
    price = df.copy()
    price['bh_equity'] = starting_equity * (1+price['return'].fillna(0)).cumprod()
    ...
    metrics = im.metrics(df=price,investment_type = instrument_type,risk_free_rate = risk_free_investment,returns_column = 'return',starting_equity = starting_equity,high='bh_equity_high',low='bh_equity_low',close='bh_equity')
    sharpe = metrics.sharpe_ratio()
    sortino = metrics.sortino_ratio()
    omega = metrics.omega_ratio()
    calmar = metrics.calmar_ratio()
    b_return = metrics.total_return()
